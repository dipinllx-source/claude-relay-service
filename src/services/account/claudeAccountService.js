...
class ClaudeAccountService {
  ...
  // 🔁 启动迁移：把旧账号 axios 双开关默认写为 'false'
  // - 仅写缺失字段；已有任何值（包括 'true'）的账号不动
  // - idempotent：可重复运行
  async migrateAxiosOptInDefaults() {
    try {
      const accounts = await redis.getAllClaudeAccounts()
      let migrated = 0
      for (const account of accounts) {
        const needsEnabled = !Object.prototype.hasOwnProperty.call(account, 'axiosRefreshEnabled')
        const needsConfirmed = !Object.prototype.hasOwnProperty.call(
          account,
          'axiosCloudflareConfirmed'
        )
        const needsBlockedAt = !Object.prototype.hasOwnProperty.call(account, 'axiosLastBlockedAt')
        const needsBlockedReason = !Object.prototype.hasOwnProperty.call(
          account,
          'axiosBlockedReason'
        )
        if (!needsEnabled && !needsConfirmed && !needsBlockedAt && !needsBlockedReason) {
          continue
        }
        if (needsEnabled) {
          account.axiosRefreshEnabled = 'false'
        }
        if (needsConfirmed) {
          account.axiosCloudflareConfirmed = 'false'
        }
        if (needsBlockedAt) {
          account.axiosLastBlockedAt = ''
        }
        if (needsBlockedReason) {
          account.axiosBlockedReason = ''
        }
        await redis.setClaudeAccount(account.id, account)
        migrated++
      }
      logger.info(`axios opt-in defaults written to ${migrated} accounts`)
      return migrated
    } catch (error) {
      logger.error('Failed to migrate axios opt-in defaults:', error)
      return 0
    }
  }
  ...
  _classifyRefreshError(err, ctx = {}) {
    ...
    // axios 抛错时携带 categoryHint（_axiosRefresh 在 CF 命中时显式打的标）
    if (err && err.categoryHint) {
      return err.categoryHint
    }
    // axios 响应异常：先检测 Cloudflare，再走默认 4xx/5xx 分类
    if (err && err.response && err.response.status) {
      if (this._isCloudflareBlocked(err.response)) {
    ...
    // 默认作为网络问题，避免误置 status='error'
    return 'oauth_network'
  }

  // 🚧 检测 Cloudflare WAF 拦截
  // 仅在非 200 响应上判定，避免合法转发的 200 响应被误判
  _isCloudflareBlocked(response) {
    ...
    if (
      lowerHeaders['cf-mitigated'] ||
      lowerHeaders['cf-ray'] ||
      (typeof lowerHeaders['server'] === 'string' && /cloudflare/i.test(lowerHeaders['server']))
    ) {
      return true
    }
    const body = response.data
    if (typeof body === 'string' && /Cloudflare|Ray ID|Attention Required|cf-error/i.test(body)) {
      return true
    }
    return false
  }

  // 📝 提炼 Cloudflare 拦截原因描述（供 axiosBlockedReason 写入）
  _extractCloudflareReason(response) {
    const headers = (response && response.headers) || {}
    const lowerHeaders = {}
    for (const [k, v] of Object.entries(headers)) {
      lowerHeaders[k.toLowerCase()] = v
    }
    const cfRay = lowerHeaders['cf-ray']
    if (cfRay) {
      return `Cloudflare WAF blocked direct OAuth call (cf-ray=${cfRay})`
    }
    return 'Cloudflare WAF blocked direct OAuth call'
  }

  // ⏱️ Poll credentials file mtime 直到推进或超时
  // 仅 stat 文件，不写文件；返回 true=推进，false=超时
  async _pollFileMtime(filePath, baselineMtimeMs, timeoutMs = 5000, intervalMs = 100) {
    const deadline = Date.now() + timeoutMs
    while (Date.now() < deadline) {
      try {
        const st = fs.statSync(filePath)
        if (st.mtimeMs > baselineMtimeMs) {
          return true
        }
      } catch (_) {
        // 文件暂时不存在或 stat 失败：继续 poll
      }
  ...
  // 🔄 Token 刷新入口
  //
  // 流程: CLI 优先重试（最多 3 次）→ axios OAuth 兜底
  // 不允许写 .credentials.json，仅 readFileSync + statSync
  // 失败时不污染 token 字段缓存；仅按失败矩阵更新 status / errorMessage
  //
  // @param {string} accountId
  // @param {'cache_expired'|'upstream_error'|'manual_refresh'} trigger
  async refreshTokenViaCredentials(accountId, trigger = 'manual_refresh') {
    const VALID_TRIGGERS = ['cache_expired', 'upstream_error', 'manual_refresh']
    if (!VALID_TRIGGERS.includes(trigger)) {
      logger.warn(`⚠️ Invalid refresh trigger '${trigger}', falling back to 'manual_refresh'`)
    ...
    try {
      ...
      if (!lockAcquired) {
        ...
        logRefreshSkipped(accountId, accountData.name, 'claude', 'already_locked', trigger)
      ...
      logRefreshStart(accountId, accountData.name, 'claude', 'credentials_refresh', trigger)
      logger.info(
        `Starting credentials-based refresh for account: ${accountData.name} (${accountId}) [trigger=${trigger}]`
      )

      // 前置校验：路径错位 / 文件不可读 → 直接 file_path_error 处置
      let credentialsPath
      try {
        credentialsPath = await this.getCredentialsPath()
        if (!fs.existsSync(credentialsPath)) {
          throw new Error(`Credentials file not found: ${credentialsPath}`)
        }
      } catch (pathError) {
        lastErrorCategory = 'file_path_error'
        lastError = pathError
        throw pathError
      }
      ...
      for (let attempt = 1; attempt <= MAX_CLI_ATTEMPTS; attempt++) {
        ...
        try {
          baselineMtimeMs = fs.statSync(credentialsPath).mtimeMs
        } catch (_) {
          baselineMtimeMs = 0
        }
        ...
        // 等待文件 mtime 推进（最多 5s）；超时仍然继续读，由后续 == 比较判定
        await this._pollFileMtime(credentialsPath, baselineMtimeMs, 5000)

        let credentials
        try {
          credentials = await this.readCredentialsFile(credentialsPath)
        } catch (readError) {
          lastErrorCategory = 'file_path_error'
          lastError = readError
          throw readError
        ...
        if (previousAccessToken && previousAccessToken === credentials.accessToken) {
          // 当前 attempt no-op；最终 cliExhaustedCategory 在循环外按 hadAnySubprocessError 决定
          const attemptCategory = cliSubprocessError ? 'cli_subprocess' : 'cli_no_op'
          lastError =
            cliSubprocessError ||
            new Error(`CLI no-op on attempt ${attempt} (file accessToken unchanged)`)
          logger.info(
        ...
        logRefreshSuccess(accountId, accountData.name, 'claude', {
          ...
          method: `credentials_file_attempt_${attempt}`
        })
        logger.success(
          `Credentials-based refresh successful for: ${accountData.name} (${accountId}) on attempt ${attempt} - Token: ${maskToken(credentials.accessToken)}`
        )
        return {
          success: true,
          accessToken: credentials.accessToken,
      ...
      const axiosEnabled =
        accountData.axiosRefreshEnabled === 'true' &&
        accountData.axiosCloudflareConfirmed === 'true'
      ...
      // 3 次 CLI 都未变 → 进入 axios 兜底
      logger.info(
        `All ${MAX_CLI_ATTEMPTS} CLI refresh attempts produced no-op for ${accountData.name} (${accountId}), falling back to direct OAuth axios`
      )
      const cachedRefreshToken = this._decryptSensitiveData(accountData.refreshToken)
      if (!cachedRefreshToken) {
        lastErrorCategory = 'invalid_grant'
        lastError = new Error('No refresh token available for axios fallback')
        throw lastError
      }

      let axiosResult
      try {
        axiosResult = await this._axiosRefresh(accountId, cachedRefreshToken, accountData)
      } catch (axiosError) {
        lastErrorCategory = this._classifyRefreshError(axiosError)
        lastError = axiosError
        throw axiosError
      }

      if (!axiosResult || !axiosResult.success) {
        lastErrorCategory = 'oauth_network'
        lastError = new Error('axios fallback returned non-success result')
        throw lastError
      }

      return {
        success: true,
        accessToken: axiosResult.accessToken,
        expiresAt: axiosResult.expiresAt
      }
    } catch (error) {
      // 失败处置矩阵
      const category = lastErrorCategory || this._classifyRefreshError(error)
      const errorObj = lastError || error
      const accountData = await redis.getClaudeAccount(accountId)
      if (accountData) {
        logRefreshError(accountId, accountData.name, 'claude', errorObj, 1, category)
        const isAutoProtectionDisabled =
          accountData.disableAutoProtection === true || accountData.disableAutoProtection === 'true'

        // status='error' 写入路径：受 disableAutoProtection 豁免
        const writeErrorStatus = async () => {
          if (isAutoProtectionDisabled) {
            logger.info(
              `Account ${accountData.name} has auto-protection disabled, skipping error status`
            )
            upstreamErrorHelper
              .recordErrorHistory(accountId, 'claude-official', 0, `refresh_${category}`, {
                errorBody: errorObj.message
              })
              .catch(() => {})
          } else {
            accountData.status = 'error'
            accountData.errorMessage = `Refresh failed (${category}): ${errorObj.message}`
            await redis.setClaudeAccount(accountId, accountData)
          }
        }

        // temp_unavailable 冷却（与 status 是独立状态机，不受 disableAutoProtection 影响）
        const writeTempUnavailable = async () => {
          await upstreamErrorHelper
            .markTempUnavailable(accountId, 'claude-official', 503, null, {
              errorBody: errorObj.message,
        ...
        let webhookSpec = null

        if (category === 'cloudflare_blocked') {
          // CF 拦截：自动关闭 axios 开关 + 重置确认 + 记录拦截信息
          // 不写 status='error'，不发 credential webhook
          await writeTempUnavailable()
          const cfResponse = errorObj && errorObj.response ? errorObj.response : null
          accountData.axiosRefreshEnabled = 'false'
          accountData.axiosCloudflareConfirmed = 'false'
          accountData.axiosLastBlockedAt = new Date().toISOString()
          accountData.axiosBlockedReason = cfResponse
            ? this._extractCloudflareReason(cfResponse)
            : 'Cloudflare WAF blocked direct OAuth call'
          await redis.setClaudeAccount(accountId, accountData)
          webhookSpec = {
            errorCode: 'CLAUDE_REFRESH_CLOUDFLARE_BLOCKED',
            reason: `Refresh failed (cloudflare_blocked): ${accountData.axiosBlockedReason}`
          }
        } else if (category === 'oauth_network') {
          await writeTempUnavailable()
          ...
          webhookSpec = {
            errorCode: 'CLAUDE_REFRESH_CLI_NO_OP',
            reason: `Refresh failed (cli_no_op): ${errorObj.message}`
          }
        } else if (category === 'invalid_grant') {
          await writeErrorStatus()
          webhookSpec = {
            errorCode: 'CLAUDE_REFRESH_INVALID_GRANT',
            reason: `Refresh failed (invalid_grant): ${errorObj.message}`
          }
        } else if (category === 'file_path_error') {
          await writeErrorStatus()
          webhookSpec = {
            errorCode: 'CLAUDE_REFRESH_FILE_PATH_ERROR',
            reason: `Refresh failed (file_path_error): ${errorObj.message}`
          }
        } else {
          // 未知 category 兜底：保守地走短冷却，不下线
          await writeTempUnavailable()
        }
        ...
        if (webhookSpec) {
          try {
            ...
            await webhookNotifier.sendAccountAnomalyNotification({
              ...
              status: 'error',
              errorCode: webhookSpec.errorCode,
              reason: webhookSpec.reason
            })
          } catch (webhookError) {
      ...
      logger.error(
        `Failed credentials refresh for account ${accountId} [category=${category}]:`,
        errorObj
      )
      throw new Error(`all refresh paths exhausted: ${category} - ${errorObj.message}`)
    } finally {
      if (lockAcquired) {
  ...
  // 🔧 axios OAuth refresh_token grant 内部 helper
  // 由 refreshTokenViaCredentials 在 CLI 3 次未变后调用作为兜底
  // 成功时原子写 cache（accessToken / refreshToken / expiresAt / status='active' 等）
  // 失败时抛出原始错误（含 axios response），由调用方根据失败矩阵处置；不在本函数写 status='error'
  async _axiosRefresh(accountId, refreshToken, accountDataCache = null) {
    const accountData = accountDataCache || (await redis.getClaudeAccount(accountId))
    if (!accountData || Object.keys(accountData).length === 0) {
      throw new Error('Account not found')
    }
    ...
    try {
      ...
      // axios 抛错时（含 4xx/5xx 配置 throwHttpErrors）：先检测 CF 再透传
      if (axiosError && axiosError.response && this._isCloudflareBlocked(axiosError.response)) {
        axiosError.categoryHint = 'cloudflare_blocked'
      }
      throw axiosError
    }

    if (response.status !== 200) {
      const err = new Error(`axios refresh non-200: ${response.status}`)
      err.response = response
      if (this._isCloudflareBlocked(response)) {
        err.categoryHint = 'cloudflare_blocked'
      }
      throw err
    }

    logger.authDetail('Axios fallback token refresh response', response.data)
    ...
    if (
      ...
      accountData.subscriptionInfo = JSON.stringify({
    ...
    logRefreshSuccess(accountId, accountData.name, 'claude', {
      ...
      expiresAt: accountData.expiresAt,
      scopes: accountData.scopes,
      method: 'axios_fallback'
    })
    logger.success(
      `Axios fallback refresh successful for: ${accountData.name} (${accountId}) - Token: ${maskToken(access_token)}`
    )
    ...
    if (hasProfileScope) {
      try {
        ...
        logger.warn(`⚠️ Failed to fetch profile info after axios refresh: ${profileError.message}`)
      }
    }
  ...
  async createAccount(options = {}) {
    ...
    if (claudeAiOauth) {
      ...
      accountData = {
        ...
        // axios OAuth fallback 默认关闭；启用前必须勾选 Cloudflare 不拦截确认
  ...
  // 🔄 刷新Claude账户token
  // @internal
  // 仅作为兼容入口保留：早期调用方（admin 手动刷新等）可能直接调它。
  // 新刷新主路径使用 `refreshTokenViaCredentials` —— 它内部 CLI 优先 3 次，再调 `_axiosRefresh` 兜底。
  // 本函数保留独立锁与 status='error' 处置，其他直接调用此函数的代码兼容性不变。
  ...
  async getValidAccessToken(accountId) {
    try {
      ...
      if (isExpired) {
        ...
        try {
          const refreshResult = await this.refreshTokenViaCredentials(accountId, 'cache_expired')
          return refreshResult.accessToken
        } catch (refreshError) {
          logger.warn(`⚠️ Refresh failed for account ${accountId}: ${refreshError.message}`)
          // 兜底：所有刷新路径都失败时尝试返回当前缓存 token（可能仍可用一小段时间）
      ...
      const processedAccounts = await Promise.all(
        accounts.map(async (account) => {
          ...
          return {
            ...
            // axios OAuth fallback 配置（前端控制开关 + 显示拦截历史）
            axiosRefreshEnabled: account.axiosRefreshEnabled === 'true',
            axiosCloudflareConfirmed: account.axiosCloudflareConfirmed === 'true',
            axiosLastBlockedAt: account.axiosLastBlockedAt || null,
            axiosBlockedReason: account.axiosBlockedReason || ''
          }
        })
      )
  ...
  async updateAccount(accountId, updates) {
    try {
      ...
      // axios 双开关 invariant：enabled='true' 必须配 confirmed='true'
      // 取目标值（更新值优先，缺失时 fallback 到现有值）
      const targetEnabled = Object.prototype.hasOwnProperty.call(updates, 'axiosRefreshEnabled')
        ? parseBooleanLike(updates.axiosRefreshEnabled)
        : accountData.axiosRefreshEnabled === 'true'
      const targetConfirmed = Object.prototype.hasOwnProperty.call(
        updates,
        'axiosCloudflareConfirmed'
      )
        ? parseBooleanLike(updates.axiosCloudflareConfirmed)
        : accountData.axiosCloudflareConfirmed === 'true'
      if (targetEnabled && !targetConfirmed) {
        const validationError = new Error(
          'axiosRefreshEnabled requires axiosCloudflareConfirmed=true'
        )
